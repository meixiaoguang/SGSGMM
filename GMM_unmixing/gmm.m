function [A,R,w_jk,mu_jk,sigma_jk,extra] = gmm(I,M,options,I_org,Results_segment,num_segment)
...
[L,KL,H] = calc_Laplacians(I_org,options, eta, beta1, beta2, M,Results_segment,num_segment);  %Building a Laplacian matrix
if use_last_init
    load('init.mat');
    for j = 1:M
...
%     beta2_ori, rho1_ori, rho2_ori, options);
    ...
    % update A
    if ~fix_A 
        der_A = calc_der_A(A, sigma, mu_jk, sigma_jk, K_all, gamma_nk, Y, KL, beta1, options); %The derivative of the objective function with respect to A
        options.der_A = der_A;
        delta_t_A = calc_time_step_adaptive(@eval_obj_fun_A, @update_A, ...%Adjust the learning rate of A
            A, options, delta_t_A, delta_t0);
        A = update_A(A, options, delta_t_A);%UpdateA
        options.A = A;
    end
    
    %% Show intermediate results and test convergence    
    [eval_total, eval_N, eval_A, eval_R, eval_S] = calc_obj_fun(A, sigma, ...
        mu_jk, sigma_jk, K_all, Y, KL, beta1, H, rho1, rho2, w_k, options);   %objective function
...
eval_A = (beta1/2) * trace(A'*KL*A) - (options.beta3/2) * trace(ssigma); 
...
[UUU,sssigma,VVV] = svd(A,'econ'); 
val = -sum(sum(0.5 * gamma_nk .* N_nk)) + beta1/2 * trace(A'*KL*A) + options.beta3/2 * trace(sssigma);
...
[UUU1,sssigma1,VVV1] = svd(A,'econ'); 
der_A = der_A + beta1 * KL * A -options.beta3 * pinv(UUU1)' * pinv(VVV1')'; 
