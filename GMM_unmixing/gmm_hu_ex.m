function [A,R,w_jk,mu_jk,sigma_jk,extra,beta1,beta2] = gmm_hu_ex(I, endmembers, options)
...
%         save('ԭʼͼmapping.mat','mapping');
    ...
    % Y = gmm_project(Y_ori, mapping);
%     load('Result_Synthenic.mat');
    [K,w_jk,mu_jk,sigma_jk] = estimate_components(endmembers, mapping, options);
%     save('Result_Synthenic.mat','A','A_error','abs_err','error_all','K','mu_jk','sigma_jk','w_jk');
end
%  save('MuuflGulf_E_gtMARK.mat','K','w_jk','mu_jk','sigma_jk');
if options.show_fig
    names = options.names;
%   endmember_scatter_plot_end_var(Y,w_jk,mu_jk,sigma_jk,names);     %comment
end
...
% [A,R,w_jk,mu_jk,sigma_jk,extra,beta1,beta2] = automatic_params(I,M,options)
...
function [A,R,w_jk,mu_jk,sigma_jk,extra,beta1,beta2] = automatic_params(I,M,options)
init_beta1 = 10;
init_beta2 = 10;
    ...
    for i = ind1s
        for j = ind2s
            ...
            if i > 1 && i < length(beta1s) && j > 1 && ...
                ...
                show_abundances(A,60,60,title,0,0,options);
