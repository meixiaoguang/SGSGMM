function [A,w_jk,mu_jk,sigma_jk] = gmm_hu(I,M,options);
...
[L,KL,H] = calc_Laplacians(I, eta, beta1, beta2, M); %modified by mei
...
options.Y = Y;
options.beta1 = beta1;
options.beta3 = beta3;%added by mei 
options.rho1 = rho1;
options.rho2 = rho2;
options.KL = KL;
options.H = H;
...
Is = (1:N*B);
Is = repmat(reshape(Is, [B,1,N]), 1, B);                                    %排列方式confused
Is = Is(:);
...
if use_mex_optimization
    options.sigma_update_in_logmvn = sparse(Is,Js,ones(N*B*B,1),N*B,N*B);                   %?排列方式
end
...
epc = EvolveProcessCapture();                                                   %confused
epc = epc.begin(max_iter);

[beta1,beta2,beta3,rho1,rho2,options] = rescale_params(sigma_jk, beta1_ori, ...
    beta2_ori, beta3_ori, rho1_ori, rho2_ori, options);
...
%     beta2_ori, rho1_ori, rho2_ori, options);
    ...
    % update w_k
    if ~fix_w_k
        w_k = sum(gamma_nk, 1) / N;                                                                      %goto here w_k = \pi_k
        options.w_k = w_k;
    end
        
    % update mu
    if ~fix_mu_jk
        der_mu = calc_der_mu(A, sigma, mu_jk, sigma_jk, K_all, gamma_nk, Y, rho1, H, w_k); %goto here
        options.der_mu = der_mu;
        delta_t_mu = calc_time_step_adaptive_mu(@eval_obj_fun_mu, @update_mu, ...
    ...
    % update A
    if ~fix_A %按矩阵方式进行优化
        der_A = calc_der_A(A, sigma, mu_jk, sigma_jk, K_all, gamma_nk, Y, KL, beta1, options);%求目标函数关于A的导数
        options.der_A = der_A;
        delta_t_A = calc_time_step_adaptive(@eval_obj_fun_A, @update_A, ...%调整A的学习率
            A, options, delta_t_A, delta_t0);
        A = update_A(A, options, delta_t_A);%更新A
        options.A = A;
    end
    
    %% Show intermediate results and test convergence    
    [eval_total, eval_N, eval_A, eval_R, eval_S] = calc_obj_fun(A, sigma, ...
        mu_jk, sigma_jk, K_all, Y, KL, beta1, H, rho1, rho2, w_k, options);%这里面代码没有改
...
%         start_sigma = 1;
    ...
    %         figure('name','w_k values'); plot(w_k1);
            ...
            KL = L - beta2_decay * beta2/beta1 * speye(N); %modified by mei。
            
            %options.beta3 = 0.9 * options.beta3;
    ...
    if show_fig
      epc = epc.updateEndmVar(Y,A,rows,cols,w_jk,mu_jk,sigma_jk,names,iter);  %comment
    end
...
% for j = 1:M
%     mu_jk{j} = gmm_project(mu_jk{j}, mapping);
%     sigma_jk{j} = mapping.M'*sigma_jk{j}*mapping.M;
% end
% 
% epc = epc.end(iter);
% 
...
[W,Neighbors] = image2graph(I,eta,1e-9);%在当前点的上下左右四个点间构建W矩阵

D = diag(sum(W,2));
L = D - W;
L = sparse(L);

KL = L - beta2/beta1 * speye(N);%modified by mei
...
function [beta1,beta2,beta3,rho1,rho2,options] = rescale_params(sigma_jk, ...
    beta1, beta2, beta3, rho1, rho2, options)
max_eig = -Inf;
...
eval_N = -sum(calc_log_gmm(Y, w_k, mu_nk, sigma_nk, options));         %对应公式18

eval_R = calc_prior_mu(w_k, mu_jk, K_all, H, rho1);
eval_S = calc_prior_sigma(w_k, sigma_jk, K_all, rho2);


[UU,ssigma,VV] = svd(A,'econ'); %modified by mei
eval_A = (beta1/2) * trace(A'*KL*A) - (options.beta3/2) * trace(ssigma); %modified by mei;
...
[UUU,sssigma,VVV] = svd(A,'econ'); %modified by mei
val = -sum(sum(0.5 * gamma_nk .* N_nk)) + beta1/2 * trace(A'*KL*A) + options.beta3/2 * trace(sssigma);%modified by mei
...
temp = zeros(M,B,K1);
for k = 1:K1
    temp(:,:,k) = A' * lambda_nk(:,:,k) - rho1 * w_k(k) * H * mu_all(:,:,k)';               %对应GMM(29)
end

for j = 1:M
    for k = 1:K(j)
        der_mu{j}(k,:) = -sum(temp(j,:,K_all(:,j)==k), 3)';                                 %对应GMM(29)
    end
end
...
der_A = zeros(N,M);
temp1 = zeros(N,M);
for k = 1:K1%公式31，循环减法，涵盖了求和过程
    der_A = der_A - lambda_nk(:,:,k) * mu_all(:,:,k);
    temp1 = temp1 - psi_k(:,:,k)' * sigma_all(:,:,k);
end
der_A = der_A + 2 * A .* temp1;
[UUU1,sssigma1,VVV1] = svd(A,'econ'); %modified by mei
der_A = der_A + beta1 * KL * A -options.beta3 * pinv(UUU1)' * pinv(VVV1')'; %modified by mei
